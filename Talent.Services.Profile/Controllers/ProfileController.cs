...
namespace Talent.Services.Profile.Controllers
{
    ...
    public class ProfileController : Controller
    {
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "talent")]
        public async Task<ActionResult> UpdateProfilePhoto([FromForm]IFormFile profilePhoto)
        {
            //Your code here;
            if (ModelState.IsValid)
            {
                if (await _profileService.UpdateTalentPhoto(_userAppContext.CurrentUserId, profilePhoto))
                {
